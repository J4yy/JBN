 ...
 * @return
 */
function detail_product($shops, $data_content, $data_unit, $data_others, $array_other_view, $content_comment, $compare_id, $popup, $idtemplate, $array_keyword)
{
    ...
    // parse for version 4.3.10
    foreach ($shops as $shop) {
        if (empty($shop['image'])) {
            $shop['image'] = NV_BASE_SITEURL . NV_UPLOADS_DIR . "/{$module_file}/default_shop.jpg";
        }
        $xtpl->assign('SHOP', $shop);
        $xtpl->parse('main.buy_offline.shop_loop');
    }
    $xtpl->parse('main.buy_offline');
