...
if (nv_user_in_groups($global_array_shops_cat[$catid]['groups_view'])) {
    ...
    // show table shops_rows for version 4.3.10
    $shops = $db->query("SELECT s.name, s.phone, s.address
        FROM nv4_shops_shops_rows AS sr
            LEFT JOIN nv4_shops_rows AS r ON r." . NV_LANG_DATA . "_alias = {$db->quote($alias_url)}
            LEFT JOIN nv4_shops_shops AS s ON s.id=sr.shop_id
        WHERE sr.row_id=r.id AND s.status=1
    ")->fetchAll();

    $contents = detail_product($shops, $data_content, $data_unit, $data_others, $array_other_view, $content_comment, $compare_id, $popup, $idtemplate, $array_keyword);
} else {
    $nv_redirect = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name;
