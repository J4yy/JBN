...
function insertCat($tentheloai)
{
    ...
    $sth = $db->prepare("INSERT INTO {$theloai} (tenTheLoai, Add_time, Update_time) VALUES (?, ?, ?)");
    $sth->execute([$tentheloai, Time(), Time()]);
    return true;
}
...
function deleteCat($number)
{
    ...
    $id = (int)$number;
    $theloai = NV_PREFIXLANG . '_' . $module_data . '_theloai';
    $sth = $db->prepare("DELETE FROM {$theloai} WHERE IdTheLoai = :id");
    $sth->execute(['id' => $id]);
    return $sth->rowCount();
}
